        ...
        <div class="word_mouth">
          <div v-for="(item,index ) in comment" :key="index">
            <div class="flex-align-items head_name">
              <img class="head_img" :src="(item.user&&item.user.avatar)?item.user.avatar:''">
              <span>{{(item.user&&item.user.nickname)?item.user.nickname:''}}</span>

              <div style="margin-top:7px">
                <van-rate v-model="item.star" disabled disabled-color="#FFB10E" :size="15"/>
              </div>
            </div>
            <div>
...
export default {
  ...
  methods: {
    allComment() {
      ...
      this.axios
        .get(this.API + "/api/v1/goods/comment/list", {
          params: {
        ...
        .then(res => {
          ...
          if (resdata.code == 200) {
            ...
            for (let v of this.comment) {
              console.log(v.user, "vvvv");
            }
