<template>
  <div
    ...
    :style="{height: calendarHeight == -100 ? 'calc(100% - 30px)':calendarHeight + 'px' }"
  ...
  methods: {
    ...
    move(e) {
      ...
      if (this.can_change_mode) {
        if (
          Math.abs(touch.clientX - this.move_start_x) >
          Math.abs(touch.clientY - this.move_start_y)
        ) {
          this.move_mode = 1;
        } else {
          this.move_mode = 2;
      ...
      if (this.move_mode == 2) {
        ...
        if (touch.clientY > this.move_start_y) {
          ...
          this.calendarMove.y = -(this.line_num * 40 > move_width
            ? this.line_num * 40 - move_width
            : 0);
        } else {
          this.calendarHeight = this.move_start_calendarHeight - move_width;
          this.calendarMove.y = -(move_width <= this.line_num * 40
            ? move_width
            : this.line_num * 40);
        }
      }
